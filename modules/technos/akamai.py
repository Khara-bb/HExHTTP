        ...
        try:
            ...
            if res in req.headers:
                if pgl == "akamai-x-get-extracted-values":
                    ...
                    segments = [
                        f"{name}={colour(value)}{value}{RESET}"
...
def req_smuggling(url, s):
    ...
    try:
        ...
        if response.status_code > 500 and response.status_code != res_main.status_code:
            print(
                f"  \033[33m └── [INTERESTING BEHAVIOR]\033[0m | {url} [{res_main.status_code} > {response.status_code}]\n     └── H {headers}\n     └── B {body}"
            )
    except:
        # traceback.print_exc()
        pass
    ...
    try:
        ...
        for h in response.headers:
            if (
                ...
                print(
                    f"  \033[33m └── [INTERESTING BEHAVIOR]\033[0m | {url} \n   └── H {headers}\n  "
                )
        ...
        for ctrl in CTRL_HEADERS:
            ...
            try:
                ...
                if "NoSuchBucket" in resp:
                    print(
                        f"\033[33m   [+] POSSIBLE CP – origin reached {fake_host} with {hdr_name!r}\033[0m"
                    )
