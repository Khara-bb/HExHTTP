    ...
    try:
        ...
        for path in test_paths:
            ...
            if ip_resp:
                ...
                if comparison:
                    print(
                        f" │  └─ \033[33m[IP+HOST]\033[0m {comparison['url1']} <> {comparison['url2']}"
                    )
                    print(
        ...
        for vh in original_vhosts:
            try:
                ...
                if req_vh.status_code not in [404, 403, 425, 503, 500, 400] and len(
                    ...
                    print(
                        f" │  └─ \033[32m[ORIGINAL]\033[0m {url} [{len(baseline_resp.content)}b] <> {vh} [{len(req_vh.content)}b]"
                    )
        ...
        if rand_resp:
            ...
            if comparison:
                print(
                    f" │  └─ {Colors.YELLOW}[WILDCARD]\033[0m Wildcard detected with random host: {rand_host_name}"
                )
                print(
                    ...
                    for san in cert_info["san"][:3]:
                        if san != host and not san.startswith("*."):
                            ...
                            try:
                                ...
                                if comparison:
                                    print(
                                        f" │      \033[34m[SAN]\033[0m Different content on SAN: {san}"
                                    )
                                    results.append(comparison)
                            except Exception:
                                continue
