        ...
        if len(hosts_list) >= 3:
            if result_key not in displayed_groups:
                ...
                if len(result_key) == 3:  # redirect case
                    ...
                    print(
                        f" ├── Host: {first_host:<25}{'→':^3} {status_code:>3}{'→':^3}{location} ({Colors.CYAN}+{len(hosts_list)-1} similar{Colors.RESET})"
                    )
                else:  # normal case
                    status_code, content_length = result_key
                    print(
                        f" ├── Host: {first_host:<25}{'→':^3} {status_code:>3} [{content_length} bytes] ({Colors.CYAN}+{len(hosts_list)-1} similar{Colors.RESET})"
                    )
            ...
            for lt in hosts_list:
                if len(result_key) == 3:  # redirect case
                    status_code, _, location = result_key
                    print(
                        f" ├── Host: {lt:<25}{'→':^3} {status_code:>3}{'→':^3}{location}"
                    )
                else:  # normal case
                    status_code, content_length = result_key
                    print(
                        f" ├── Host: {lt:<25}{'→':^3} {status_code:>3} [{content_length} bytes]"
                    )
