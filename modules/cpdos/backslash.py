...
def backslash_test(pp, url_b, req_main, s):
    ...
    if req_b.status_code != req_main.status_code:
        print(
            f" {Identify.behavior} | {VULN_NAME} {req_main.status_code} > {req_b.status_code} | CACHETAG : {cache_status} | \033[34m{url_b}\033[0m | PAYLOAD: {Colors.THISTLE}{pp}{Colors.RESET}"
        )
        vcp_c = vcp_code(url_b, s, req_b)
        if vcp_c:
            print(
                f" {Identify.confirmed} | {VULN_NAME} {req_main.status_code} > {req_b.status_code} | CACHETAG : {cache_status} | \033[34m{url_b}\033[0m | PAYLOAD: {Colors.THISTLE}{pp}{Colors.RESET}"
            )
    elif len(req_b.content) != len(req_main.content):
        print(
            f" {Identify.behavior} | {VULN_NAME} {len(req_main.content)}b > {len(req_b.content)}b | CACHETAG : {cache_status} | \033[34m{url_b}\033[0m | PAYLOAD: {Colors.THISTLE}{pp}{Colors.RESET}"
        )
        vcp_l = vcp_len(url_b, s, req_b)
        if vcp_l:
            print(
                f" {Identify.confirmed} | {VULN_NAME} {len(req_main.content)}b > {len(req_b.content)}b | CACHETAG : {cache_status} | \033[34m{url_b}\033[0m | PAYLOAD: {Colors.THISTLE}{pp}{Colors.RESET}"
            )
