...
def valid_cache(uri, req, req_h, headers):
    ...
    if (
        ...
        print(
            f" {Identify.confirmed} | CVE-2025-49826 | \033[34m{uri}\033[0m | {req.status_code} > {req_h.status_code} | PAYLOAD: {headers}"
        )
    elif (
        req_confirm.status_code == req_verify.status_code
        and req_confirm.status_code != req.status_code
        ...
        print(
            f" {Identify.behavior} ++ | CVE-2025-49826 | \033[34m{uri}\033[0m | {req.status_code} > {req_h.status_code} | PAYLOAD: {headers}"
        )
    else:
        print(
            f" {Identify.behavior} | CVE-2025-49826 | \033[34m{uri}\033[0m | {req.status_code} > {req_h.status_code} | PAYLOAD: {headers} "
